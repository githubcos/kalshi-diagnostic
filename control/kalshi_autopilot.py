...
def apply_profile_once():
    rundir=get_run_dir(); sentinel=rundir/'profile_applied_high_volume_v3'
    ...
    backup=rundir/'config_before_high_volume_v3.json'; shutil.copy2(cfgp,backup); os.chmod(backup,0o600); changed=[]
    ...
    (rundir/'settings_high_volume_v3.txt').write_text('UTC='+utc()+'\nMODE=PAPER_HIGH_VOLUME_DATA_COLLECTION\nSTART_BALANCE=100\nWIN_BALANCE=200\nLOSS_BALANCE=0\n'+'\n'.join(changed)+'\n'); sentinel.write_text(utc()+'\n')
    (REPO/'docs'/'agent'/'paper_stress_config.txt').write_text('UTC='+utc()+'\nLIVE_ALLOWED=false\nRUN_DIR='+str(rundir)+'\nPROFILE=HIGH_VOLUME_DATA_COLLECTION_V3\n'+'\n'.join(changed)+'\n'); write_status('PAPER_STRESS_CONFIG',100,'PASS','High-volume PAPER v3 applied: trade size raised to $5 to avoid the $2 hedge remainder floor. Start=$100, terminal=$200/$0, logs preserved.','START_PAPER'); return rundir
def latest_balance():
    p=BOT/'polyarb.log'
...
def publish():
    ...
    run(['git','commit','-m','Update PAPER high-volume stress telemetry v3'],REPO,60); run(['git','pull','--rebase','--autostash','origin','main'],REPO,120); run(['git','push','origin','HEAD:main'],REPO,120)
def main():
    ...
    while True:
        ...
        if state=='RUNNING': write_status('PAPER_HIGH_VOLUME_STRESS',50,'RUNNING',f'High-volume PAPER v3 running; current={"unknown" if bal is None else f"${bal:.2f}"}. Logs preserved.','CONTINUE_UNTIL_200_OR_0')
